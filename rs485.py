...
import logging
logging.basicConfig(filename='/home/pi/Rasberry-Pi/rs485.log',level=logging.DEBUG)

#initialize port
try:
	#Define port and slave address(decimal) here
	instr = minimalmodbus.Instrument("/dev/ttyAMA0", 1)
	logging.info("Port intialized, connected to /dev/ttyAMA0. ")
except Exception, e:
	logging.debug(str(e))
...
try:
	...
	client.connect()
	myQosLevel=1
	logging.info("IBM IoTF connected succesfully, QoS Level at '%s'") %(myQosLevel)
except Exception, e:
	logging.debug(str(e))
	print str(e)
